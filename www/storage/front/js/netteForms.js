...
var LiveForm = {
    options: {
        pairErrorClass: 'has-error',            // CSS class for an invalid control
        controlErrorClass: 'form-control-error',            // CSS class for an invalid control
        errorMessageClass: 'help-block',            // CSS class for an error message
...
LiveForm.setUpHandlers = function(el) {
    ...
    el.onkeydown = function (event) {
        if (self.options.wait >= 200) {
            ...
            $(this).closest('.form-group').removeClass(self.options.pairErrorClass);
...
LiveForm.addError = function(el, message) {
    ...
    $(el).closest('.form-group').addClass(this.options.pairErrorClass);
...
LiveForm.removeError = function(el) {
    $(el).closest('.form-group').removeClass(this.options.pairErrorClass);
...
Nette.validateForm = function(sender) {
    ...
    for (i = 0; i < form.elements.length; i++) {
        ...
        if (!Nette.validateControl(elem)) {
            var $pair = $(form).find('.has-error').filter(":first");
            $.scrollTo( $pair, 500 );
            $pair.filter('.form-control').focus();
            return false;
        }
    }
    return true;
